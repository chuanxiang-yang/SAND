...
p.add_argument('--octree_depth', type=int, default=9,
               help='epoch frequency to update tensorboard summary')
...
def train():
    ...
    log_filename = os.path.join(opt.root_path,'out_train.log')
    ...
    training.train(model=model, sdf_dataset=sdf_dataset,train_dataloader=dataloader, steps=opt.num_steps,
                   ...
                   use_lr_scheduler=True, is_wire=(opt.model_type=='wire' or opt.model_type=='wf'), log_file=log_file, device=device)


def init_dataloader(opt):
    ''' load sdf dataloader via eikonal equation or fitting sdf directly '''

    sdf_dataset = dataio.MeshSDF(opt.mesh_path,octree_depth=opt.octree_depth)
