...
class CoordinateNet(nn.Module):
    ...
    def forward(self, model_input, layers=None):

        coords = model_input['coords']
        output = self.net(coords, layers)
...
class FCBlock(nn.Module):
    ...
    def __init__(self, in_features, out_features,
        ...
        setattr(self, "out_layer0", nn.Linear(num_hidden_features, out_features))
        getattr(self, 'out_layer0').apply(self.weight_init)

        for i in range(num_hidden_layers):
            self.net.append(nn.Sequential(
                nn.Linear(hidden_features[i], hidden_features[i+1]), copy.deepcopy(nl)
            ))
            setattr(self, "out_layer" + str(i+1), nn.Linear(num_hidden_features, out_features * self.poly))
            getattr(self, 'out_layer' + str(i+1)).apply(self.weight_init)
    ...
    def forward(self, coords, layers=None):
        if layers is None:
            feature = coords
            preds = []
            for i, module in enumerate(self.net):
                feature = module(feature)
                residual = getattr(self, 'out_layer' + str(i))(feature)
                residual = residual.reshape(*residual.shape[0:-1], self.out_features, -1)
                residual = torch.prod(residual, dim=-1)
                preds.append(residual)
            return preds
        else:
            helper_layers = layers
            feature = coords
            preds = []

            for i, module in enumerate(self.net):
                feature = module(feature)
                residual = getattr(self, f"out_layer{i}")(feature)
                residual = residual.view(*residual.shape[0:-1], self.out_features, -1)
                residual = torch.prod(residual, dim=-1)
                preds.append(residual)
                mask = helper_layers > len(preds)
                feature = feature[mask]
                helper_layers = helper_layers[mask]
                if helper_layers.sum() == 0:
                    break
            return preds
