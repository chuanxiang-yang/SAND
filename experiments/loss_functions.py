...
def overfit_sdf_loss_total(model_output, gt, is_multiscale, lambda_grad=1e-3,
    ...
    gt_sdf = gt['sdf'].squeeze()
    pred_sdf = model_output['model_out']
    task_num = len(pred_sdf)
    losses = []
    total_loss = torch.tensor(0,device=gt_sdf.device)
    for i in range(task_num):
        if i == 0:
            pred = pred_sdf[i].squeeze()
        else:
            pred = pred + pred_sdf[i].squeeze()



        loss = ((gt_sdf - pred).abs() ** 1).sum()
        loss = loss / len(gt_sdf)
        losses.append(loss)
